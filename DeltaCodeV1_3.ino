...
2018-03-17
*/
// Trapez profil motion is not smooth!
...
bool Actuators::moveJ(float stepsL, float stepsR){ // Quasi-simultaneous operation of the two motors (Both motors stop at the same time) with trapez profile
	...
	for(int i = 0; i < int(round(minSteps)); i++){
		...
		// Moving the left motor:
		steps2TakeL += stepRatioL;
		int integerStepL = int(round(steps2TakeL));
		for( int j = 0; j < integerStepL;  j++ ){
			// Trapeze profile ( left motor ):
