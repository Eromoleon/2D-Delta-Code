...
MotorAngles angles;
Actuators actuators(motorL, motorR, magnetP);
...
void setup() {
	...
	motorL->setSpeed(10);  // 10 rpm   
	
	//actuators.moveJ(90, 100);
...
bool Actuators::moveJ(float thetaL, float thetaR){
	...
	if(stepsL > stepsR){
		if (thetaR == 0){
			...
			motorL->step(int(round(stepsL)), FORWARD,SINGLE);
		}
		else{
			float stepRatio = stepsL / stepsR;
			float stepRatioR = 1;
			...
			for(int i = 0; i < int(round(stepsR)); i++){
				...
				motorR->step(integerStepR,FORWARD,SINGLE);
				...
				motorL->step(integerStep, FORWARD, SINGLE);
		...
		if (thetaL == 0){
			motorR->step(stepsR, FORWARD,SINGLE);
		}
		else{
			
			float stepRatio = stepsR / stepsL;
			float steps2Take = 0.0;
			float difference = 0.0;
			for(int i = 0; i < stepsL; i++){
				...
				steps2Take += stepRatio;
				int integerStep = round(steps2Take);
				
				motorR->step(integerStep, FORWARD, SINGLE);
				steps2Take -= integerStep;
			}
		}
	}
